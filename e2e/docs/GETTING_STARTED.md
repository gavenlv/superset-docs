...
Examples 是 Superset 内置的示例数据（婴儿姓名、能源结构、销售数据等），用于演示各种图表类型和功能。默认情况下，Superset 从 GitHub 下载这些数据，但在网络受限的企业环境中会失败。

本项目通过将 `example-data/` 目录挂载到容器，实现完全离线加载。详细指南请参见：[OFFLINE_EXAMPLES.md](./OFFLINE_EXAMPLES.md)
